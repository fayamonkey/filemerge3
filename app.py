        ...
        if st.session_state.conversion_done:
            ...
            try:
                ...
                with col3:
                    if st.session_state.chunking_enabled:
                        ...
                        st.metric("Output Format", "Whole Documents")
            except:
                pass
            ...
            with st.expander("Preview Converted Content", expanded=True):
                if st.session_state.output_format == "Single Merged File":
                    ...
                    st.text_area("Merged Content Preview", preview_text, height=300)
                else:
                    if st.session_state.processed_files:
                        sample_file = st.session_state.processed_files[0]
                        preview_text = sample_file[0][:2000]
                        if len(sample_file[0]) > 2000:
                            preview_text += "...(content truncated for preview)"
                        st.text_area(f"Preview of {sample_file[1]}", preview_text, height=300)
